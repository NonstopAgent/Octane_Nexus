...
export async function GET(req: NextRequest) {
  ...
  try {
    while (hasTimeBudget(Date.now() - startedAt)) {
      ...
      for (const job of jobs) {
        ...
        try {
          ...
          // Reusing an existing brief is the idempotency guard working as
          // designed, not a failure — count it separately so a cheap run is
          // distinguishable from one that actually produced nothing.
    ...
    if (processed.length > 0 && generated === 0 && reused === 0) {
      console.error(
        `[brief-worker] processed ${processed.length} job(s) and produced nothing`,
        { date: briefDate, skipped, retried, failed, queue }
      );
    } else if (processed.length > 0) {
      console.info(
        `[brief-worker] ${generated} generated, ${reused} reused across ${processed.length} job(s)`,
        { date: briefDate, queue }
      );
    }

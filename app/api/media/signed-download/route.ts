...
/**
 * GET /api/media/signed-download?path=&filename=
 * Returns a short-lived signed URL with download disposition.
 * Ownership: same as signed-url (clips, uploads, or content_posts).
 */
export async function GET(req: NextRequest) {
  try {
    ...
    const filename = req.nextUrl.searchParams.get('filename')?.trim() || undefined;
    ...
    const { data, error } = await client.storage
      .from(bucket)
      .createSignedUrl(path, EXPIRE_SEC, { download: filename ?? true });

    if (error) {
      console.error('media/signed-download error:', error);
    ...
    console.error('media/signed-download error:', e);

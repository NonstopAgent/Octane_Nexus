...
export async function refreshTrackedChannelsVideos(
  ...
  // Skip channels synced recently. This used to run unconditionally on every
  // invocation, spending real YouTube API quota per tracked channel each time.
  // A daily job never needs to re-sync something fetched an hour ago, so this
  // costs nothing in normal operation and bounds the damage if the worker is
  // triggered repeatedly.
  const STALE_AFTER_MS = 6 * 60 * 60 * 1000;
  const nowMs = Date.now();
  const stale = rows.filter((row) => {
    if (!row.last_synced_at) return true;
    const age = nowMs - new Date(row.last_synced_at as string).getTime();
...
export type BriefRunResult = {
  /** True when the user now has a brief for the date — freshly made or reused. */
  generated: boolean;
  /** True when an existing brief was reused instead of paying for Gemini again. */
  reused: boolean;
  channelsRefreshed: number;
  channelsAlreadyFresh: number;
...
export async function runBriefForUser(
  ...
  const { refreshed, failed, skipped } = await refreshTrackedChannelsVideos(
    admin,
    userId
  );
  ...
  // No force flag: if today's brief already exists it is reused rather than
  // paying for another Gemini call, which is what makes re-running the worker
  // cheap and safe.
